...
@section('title', $category->description->meta_title ?: system_setting('base.meta_title', 'BeikeShop开源好用的跨境电商系统') . ' - ' . $category->description->title)
          ...
          <div class="col-lg-6">
            <div class="relative">
              <div class="text-start">
                <h1 class="fs-96 text-uppercase fs-sm-10vw mb-0 lh-1">{{ $category->description->title }}</h1>
              </div>
            </div>
          </div>

          <div class="col-lg-4">
            <p class="mb-0">{{ $category->description->summary ?: (system_setting('base.meta_description') ?: 'Explore the latest stories, insights, and updates from Banli.') }}</p>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="row g-4">
          @foreach ($category_pages as $page)
            ...
            <div class="col-lg-4 col-md-6">
              <a href="{{ type_route('page', $page) }}" class="d-block hover relative rounded-20 overflow-hidden text-light banli-news-card">
