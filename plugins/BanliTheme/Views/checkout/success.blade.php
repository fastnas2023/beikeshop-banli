...
<div class="container">
  <div class="card glass-card mt-lg-5 mt-3 w-max-1000 mx-auto">
    <div class="card-body p-0">
      ...
      <div class="checkout-success__body">
        <div class="mt-4">
          <div class="card glass-card mb-2 order-head">
            <div class="card-header border-secondary d-flex align-items-center justify-content-between">
              <h5 class="card-title text-light">{{ __('shop/account/order_info.order_details') }}</h5>
              @if (current_customer())
                <a class="btn btn-sm btn-neon" href="{{ shop_route('account.order.show', $order->number) }}">{{ __('common.view_more') }}</a>
                @else
                <a class="btn btn-sm btn-neon" href="{{ shop_route('orders.show', ['number' => $order->number, 'email' => $order->email]) }}">{{ __('common.view_more') }}</a>
              @endif
            </div>
            <div class="card-body">
              <div class="bg-dark p-2 table-responsive rounded">
                @if (!is_mobile())
                  <table class="table table-borderless table-dark text-light mb-0">
                    <thead>
                      <tr>
          ...
          <div class="card glass-card mt-4">
            <div class="card-header border-secondary"><h5 class="card-title text-light">{{ __('order.address_info') }}</h5></div>
            <div class="card-body">
