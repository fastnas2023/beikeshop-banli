<div id="checkout-address-app" v-cloak>
  <div class="checkout-black glass-card p-3 rounded mb-4" v-if="shippingRequired">
    <div class="checkout-title">
      <div class="d-flex">
        <h5 class="mb-0 me-4 text-light">{{ __('shop/checkout.address') }}</h5>
      ...
      <div class="row">
        <template v-if="source.isLogin">
          <div class="col-lg-6 col-12" v-for="address, index in source.addresses" :key="index"
            ...
            <div :class="['item glass-card rounded-2 overflow-hidden', address.id == form.shipping_address_id ? 'active border-neon' : 'border-secondary']"
          ...
          <div class="col-lg-6 col-12" v-if="!isAllAddress">
            <div class="item address-right glass-card border-secondary">
              <button class="btn btn-outline-light w-100 mb-md-3" v-if="source.addresses.length > 1"
                @click="isAllAddress = true">{{ __('shop/checkout.choose_another_address') }}</button>
              <button class="btn btn-outline-light w-100" @click="editAddress(null, 'shipping_address_id')"><i
        ...
        <template v-else>
          <div class="col-lg-6 col-12" v-if="source.guest_shipping_address">
            <div class="item active glass-card border-neon">
          ...
          <div class="col-lg-6 col-12" v-if="!source.guest_shipping_address">
            <div class="item address-right glass-card border-secondary">
              <button class="btn btn-outline-light w-100" @click="editAddress(null, 'guest_shipping_address')"><i
                  class="bi bi-plus-square-dotted"></i> {{ __('shop/checkout.add_new_address') }}</button>
            </div>
          </div>
        </template>
  ...
  <div class="checkout-black glass-card p-3 rounded mb-4" v-if='!this.shippingRequired || !same_as_shipping_address'>
    <div class="checkout-title">
      <div class="d-flex">
        <h5 class="mb-0 me-4 text-light">{{ __('shop/checkout.payment_address') }}</h5>
      ...
      <div class="row">
        <template v-if="source.isLogin">
          <div class="col-lg-6 col-12" v-for="address, index in source.addresses" :key="index"
            ...
            <div :class="['item glass-card rounded-2 overflow-hidden', address.id == form.payment_address_id ? 'active border-neon' : 'border-secondary']"
          ...
          <div class="col-lg-6 col-12" v-if="!isAllAddressPayment">
            <div class="item address-right glass-card border-secondary">
              ...
              <button class="btn btn-outline-light w-100" @click="editAddress(null, 'payment_address_id')"><i
                  class="bi bi-plus-square-dotted"></i> {{ __('shop/checkout.add_new_address') }}</button>
            </div>
          </div>
        </template>
        <template v-else>
          ...
          <div class="col-lg-6 col-12" v-if="!source.guest_payment_address">
            <div class="item address-right glass-card border-secondary">
              <button class="btn btn-outline-light w-100" @click="editAddress(null, 'guest_payment_address')"><i
                  class="bi bi-plus-square-dotted"></i> {{ __('shop/checkout.add_new_address') }}</button>
            </div>
          </div>
        </template>
