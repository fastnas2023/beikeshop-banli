<div class="module-item {{ $design ? 'module-item-design' : ''}}" id="module-{{ $module_id }}">
  <section id="section-products" class="bg-dark section-dark text-light" style="{{ !empty($content['bg_color']) ? 'background-color: '.$content['bg_color'].'!important;' : '' }}">
    <div class="container">
      <div class="row g-4 gx-5 justify-content-center">
        <div class="col-lg-6 text-center">
          <div class="subtitle s2 mb-3 wow fadeInUp" data-wow-delay=".0s" style="{{ !empty($content['text_color']) ? 'color: '.$content['text_color'].';' : '' }}">
            {{ !empty($content['sub_title']) ? (is_array($content['sub_title']) ? ($content['sub_title'][locale()] ?? $content['sub_title']['en'] ?? '') : $content['sub_title']) : __('shop/products.hot_products') }}
          </div>
          <h2 class="wow fadeInUp" data-wow-delay=".2s" style="{{ !empty($content['text_color']) ? 'color: '.$content['text_color'].';' : '' }}">
            {{ !empty($content['title']) ? (is_array($content['title']) ? ($content['title'][locale()] ?? $content['title']['en'] ?? '') : $content['title']) : __('BanliTheme::common.featured_products') }}
          </h2>
        </div>
      </div>
