@extends('layout.master')

@section('body-class', 'page-account-order-list')

@section('content')
<div class="bg-dark section-dark text-light pb-5" style="padding-top: 100px; min-height: 100vh;">
  <x-shop-breadcrumb type="static" value="account.order.index" />

  <div class="container">
    <div class="row">
      <x-shop-sidebar />


      <div class="col-12 col-md-9">
          @if (!is_mobile())
          <div class="card glass-card shadow-sm mb-4 account-card order-wrap h-min-600 bg-dark bg-opacity-75 border-secondary text-light">
            <div class="card-header border-secondary d-flex justify-content-between align-items-center">
              <h5 class="card-title">{{ __('shop/account/order.index') }}</h5>
            </div>
          <div class="card-body">
            @include('account.order_status')
            <div class="table-responsive">
              <table class="table table-dark bg-transparent border-secondary text-light">
                @if (count($orders))
                @hookwrapper('account.order.table_header', $orders)
                  @foreach ($orders as $order)
                    <tbody>
                      <tr class="sep-row"><td colspan="4"></td></tr>
                      <tr class="head-tr">
                        <td colspan="4">
                          <span class="order-created me-4">{{ $order->created_at }}</span>
                          <span
                            class="order-number">{{ __('shop/account/order.order_number') }}：{{ $order->number }}</span>
                        </td>
                      </tr>
                      @foreach ($order->orderProducts as $product)
                        <tr class="{{ $loop->first ? 'first-tr' : '' }}">
                          <td>
                            <div class="product-info">
                              <div class="img border d-flex justify-content-center align-items-center wh-60"><img src="{{ image_resize($product->image) }}" alt="{{ $product->name }}" class="img-fluid"></div>
                              <div class="name w-max-600">
                                <a class="text-light" href="{{ shop_route('products.show', ['product' => $product->product_id, 'variant' => $product->product_sku]) }}">{{ $product->name }}</a>
                                <div class="quantity mt-1 text-white-50">x {{ $product->quantity }}</div>
                                @hook('account.order.list.name.after', $product)
                              </div>
                            </div>
                          </td>
                          @if ($loop->first)
                            <td rowspan="{{ $loop->count }}">
                              {{ currency_format($order->total, $order->currency_code, $order->currency_value) }}</td>
                            <td rowspan="{{ $loop->count }}">
                              @if ($order->status == 'unpaid')
                              <span class="text-danger">{{ $order->status_format }}</span>
                              @elseif ($order->status == 'cancelled')
                              <span class="text-secondary">{{ $order->status_format }}</span>
                              @else
                              <span class="text-success">{{ $order->status_format }}</span>
                              @endif
                            </td>
                            <td rowspan="{{ $loop->count }}" class="text-end">
                              @hook('account.order.actions', $order)

                              <a href="{{ shop_route('account.order.show', ['number' => $order->number]) }}" class="btn btn-outline-secondary btn-sm mb-2 w-100">{{ __('shop/account/order.check') }}</a>
                              @if ($order->status == 'unpaid')
                                <a href="{{ shop_route('orders.pay', $order->number) }}" class="btn w-100 btn-neon btn-sm nowrap mb-2">{{ __('shop/account/order_info.to_pay') }}</a>
                              @endif

                              @if ($order->status == 'unpaid' || $order->status == 'paid')
                              <button class="btn btn-outline-danger btn-sm cancel-order w-100" data-number="{{ $order->number }}" type="button">{{ __('shop/account/order_info.cancel') }}</button>
                              @endif

                              @hook('account.order.actions_after', $order)
                            </td>
                          @endif
                        </tr>
                      @endforeach
                    </tbody>
                  @endforeach
                @endhookwrapper
                @else
                  <tbody>
                    <tr><td colspan="4" class="border-0"><x-shop-no-data /></td></tr>
                  </tbody>
                @endif
              </table>
            </div>

            {{ $orders->links('shared/pagination/bootstrap-4') }}
          </div>
        </div>
        @else
          {{-- Mobile View --}}
          <div class="card glass-card shadow-sm mb-4 account-card order-wrap bg-dark bg-opacity-75 border-secondary text-light">
            <div class="order-mb-wrap p-3">
          @include('account.order_status')
          @if (count($orders))
            @hookwrapper('account.order.mobile_table_header', $orders)
            @foreach ($orders as $order)
            <div class="order-mb-list card glass-card shadow-sm mb-3 bg-dark bg-opacity-75 border-secondary text-light">
              <div class="card-body">
                <div class="header-wrapper d-flex justify-content-between">
                  <div>{{ __('shop/account/order.order_number') }}：{{ $order->number }}</div>
                  <div>
                    @if ($order->status == 'unpaid')
                    <span class="text-danger">{{ $order->status_format }}</span>
                    @elseif ($order->status == 'cancelled')
                    <span class="text-secondary">{{ $order->status_format }}</span>
                    @else
                    <span class="text-success">{{ $order->status_format }}</span>
                    @endif
                  </div>
                </div>
                <div class="content-wrapper">
                  <div class="order-product-wrap mb-2" onclick="window.location.href='{{ shop_route('account.order.show', ['number' => $order->number]) }}'">
                    @foreach ($order->orderProducts as $product)
                    <div class="product-info d-flex mb-2">
                      <div class="img border d-flex justify-content-center align-items-center wh-60"><img src="{{ $product->image }}" alt="{{ $product->name }}" class="img-fluid"></div>
                      <div class="name ms-2">
                        <div>{{ $product->name }}</div>
                        <div class="quantity mt-1 text-white-50">x {{ $product->quantity }}</div>
                      </div>
                    </div>
                    @endforeach
                  </div>
                </div>
                <div class="footer-wrapper">
                  <div class="d-flex justify-content-between">
                    <div class="text-white-50">{{ $order->created_at }}</div>
                    <div class="fw-bold">{{ __('admin/order.total') }}：<span class="amount text-primary">{{ currency_format($order->total, $order->currency_code, $order->currency_value) }}</span></div>
                  </div>
                  <div class="footer-btns d-flex justify-content-end mt-2">
                    @hook('account.order.actions', $order)
                    @if ($order->status == 'unpaid')
                      <a href="{{ shop_route('orders.pay', $order->number) }}" class="btn btn-neon btn-sm nowrap">{{ __('shop/account/order_info.to_pay') }}</a>
                      <button class="btn btn-outline-danger ms-2 btn-sm cancel-order" data-number="{{ $order->number }}" type="button">{{ __('shop/account/order_info.cancel') }}</button>
                    @endif
                    <a href="{{ shop_route('account.order.show', ['number' => $order->number]) }}"
                      class="btn btn-outline-secondary btn-sm ms-2">{{ __('shop/account/order.check') }}</a>
                    @hook('account.order.actions_after', $order)
                  </div>
                </div>
              </div>
            </div>
            @endforeach
            @endhookwrapper

            {{ $orders->links('shared/pagination/bootstrap-4') }}
          @else
            <x-shop-no-data />
          @endif
        </div>
          </div>
        @endif
      </div>
    </div>
  </div>
</div>
@endsection

@push('add-scripts')
<style>
  .pagination .page-link { background-color: transparent; border-color: rgba(255,255,255,0.1); color: #fff; }
  .pagination .page-item.active .page-link { background-color: var(--bs-primary); border-color: var(--bs-primary); }
  .pagination .page-item.disabled .page-link { background-color: rgba(255,255,255,0.05); border-color: rgba(255,255,255,0.1); color: rgba(255,255,255,0.5); }
  .nav-tabs { border-bottom: 1px solid rgba(255,255,255,0.1); }
  .nav-tabs .nav-link { color: #aaa; border: none; }
  .nav-tabs .nav-link:hover { color: #fff; border-color: transparent; }
  .nav-tabs .nav-link.active { color: #fff; background-color: transparent; border-bottom: 2px solid var(--bs-primary); }
</style>
<script>
  $('.cancel-order').click(function(event) {
    var number = $(this).data('number')
    $http.post(`orders/${number}/cancel`).then((res) => {
      layer.msg(res.message)
      window.location.reload()
    })
  });
</script>
@endpush
