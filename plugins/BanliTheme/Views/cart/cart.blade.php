  ...
  <div class="container" id="app-cart" v-cloak>
    ...
    <div class="row mt-5" v-if="products.length">
      <div class="col-12 col-md-9 left-column">
        <div class="card glass-card h-min-600">
          <div class="card-body p-lg-4">
            <div class="p-lg-0"><h4 class="mb-3 text-light">{{ __('shop/carts.commodity') }}</h4></div>
              ...
              <table class="table table-dark text-light">
                ...
                <tbody>
                  <tr v-for="product, index in products" :key="index" :class="product.selected ? 'active' : ''">
                    ...
                    <td>
                      <a class="name text-truncate-2 mb-1 text-light fw-bold" :href="product.url" v-text="product.name"></a>
                      @hook('cart.product.name.after')
                      <div class="text-size-min text-secondary mb-1">@{{ product.variant_labels }}</div>
                      <div class="price text-secondary" v-html="product.price_format"></div>
                    </td>
                    <td>
                      <div class="quantity-wrap input-group">
                        <button class="btn quantity-reduce text-light" :data-id="product.sku_id" type="button"><i class="bi bi-dash-lg"></i></button>
                        <input type="text" class="form-control form-control-dark" :data-id="product.sku_id" @input="quantityChange(product.quantity, product.cart_id, product.sku_id)" onkeyup="this.value=this.value.replace(/\D/g,'')" v-model.number="product.quantity" name="quantity" minimum="1">
                        <button class="btn quantity-increase text-light" :data-id="product.sku_id" type="button"><i class="bi bi-plus-lg"></i></button>
                      </div>
                    </td>
      ...
      <div class="col-12 col-md-3 right-column">
        <div class="card glass-card x-fixed-top">
          <div class="card-body p-lg-4">
            <div class="total-wrap">
              <div class="p-lg-0"><h4 class="mb-3 text-light">{{ __('shop/carts.product_total') }}</h4></div>
              <div class="card-body p-lg-0">
                <ul class="list-group list-group-flush bg-transparent">
                  @hook('shop.cart.index.total.before')
                  <li class="list-group-item bg-transparent text-light border-secondary"><span>{{ __('shop/carts.all') }}</span><span>@{{ allProduct }}</span></li>
                  <li class="list-group-item bg-transparent text-light border-secondary"><span>{{ __('shop/carts.selected') }}</span><span>@{{ total_quantity }}</span></li>
                  <li class="list-group-item bg-transparent text-light border-bottom-0"><span>{{ __('shop/carts.product_total') }}</span><span class="total-price text-neon">@{{ amount_format }}</span></li>
                  @hook('shop.cart.index.total.after')
                  <li class="list-group-item bg-transparent d-grid gap-2 mt-3 border-bottom-0">
                    @hookwrapper('cart.confirm')
                    <button type="button" class="btn btn-neon fs-5 fw-bold" @click="checkedBtnToCheckout">{{ __('shop/carts.to_checkout') }}</button>
                    @endhookwrapper
                  </li>
                </ul>
              </div>
    ...
    <div v-else class="d-flex justify-content-center align-items-center flex-column">
      <div class="empty-cart-wrap text-center mt-5">
        <div class="empty-cart-icon mb-3 text-light">
          <i class="bi bi-cart fs-1"></i>
        </div>
        <div class="empty-cart-text mb-3">
          <h5 class="text-light">{{ __('shop/carts.cart_empty') }}</h5>
